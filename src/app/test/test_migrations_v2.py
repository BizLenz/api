...
class TestMigrationsIsolated:
    ...
    def test_models_create_tables_directly(self, mock_get_db_url, isolated_engine):
        ...
        metadata = MetaData()
        core_tables = [User.__table__, BusinessPlan.__table__, AnalysisJob.__table__, AnalysisResult.__table__]
        ...
        expected_tables = {"users", "business_plans", "analysis_jobs", "analysis_results"}
        created_core_tables = expected_tables.intersection(tables)
        
        assert len(created_core_tables) >= 2, f"핵심 테이블이 생성되지 않음. 생성된: {tables}"
