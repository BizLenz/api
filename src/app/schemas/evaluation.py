...
class SectionResult(BaseModel):
    ...
    analysis: Optional[str] = Field(None, description="Gemini 분석 내용", max_length=5000)
    strengths: Optional[List[str]] = Field(default_factory=list, description="섹션별 강점", max_items=10)
    weaknesses: Optional[List[str]] = Field(default_factory=list, description="섹션별 약점", max_items=10)
    evidence_text: Optional[str] = Field(None, description="관련 내용 발췌/요약", max_length=2000)
    ...
    def validate_score_range(self) -> Self:
        if self.score is not None:
            ...
            # 소수점 2자리 제한 (DB Numeric(5,2)와 일치)
...
class KeywordAnalysis(BaseModel):
    """키워드 분석"""
    keywords: List[str] = Field(default_factory=list, description="주요 키워드", max_items=50)
...
def evaluation_response_to_db_json(response: EvaluationResponse) -> Dict[str, Any]:
    ...
    # Decimal을 float로 변환하여 JSON 직렬화 가능하도록 처리
