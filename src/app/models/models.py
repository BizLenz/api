...
class BusinessPlan(Base):
    ...
    updated_at = Column(TIMESTAMP(timezone=True), server_default=func.now(), onupdate=func.now(), comment="수정 시각")
    ...
    __table_args__ = (
        ...
        CheckConstraint("status IN ('pending','processing','completed','failed')", name='ck_business_plans_status_valid'),
    )
...
class AnalysisJob(Base):
    ...
    retry_count = Column(Integer, server_default='0', nullable=False, comment="재시도 횟수")
    ...
    __table_args__ = (
        Index('idx_analysis_jobs_plan_type_latest', 'plan_id', 'job_type', desc('created_at')),
        Index('idx_analysis_jobs_type_completed', 'job_type', 'status', desc('completed_at')),
        ...
        CheckConstraint('token_usage >= 0', name='ck_analysis_jobs_token_usage_positive'),
        CheckConstraint('retry_count >= 0', name='ck_analysis_jobs_retry_count_positive'),
        CheckConstraint('processing_time_seconds >= 0', name='ck_analysis_jobs_processing_time_positive'),
        CheckConstraint("status IN ('pending','processing','completed','failed')", name='ck_analysis_jobs_status_valid'),
    )

    business_plan = relationship("BusinessPlan", back_populates="analysis_jobs", foreign_keys=[plan_id])
    analysis_results = relationship("AnalysisResult", back_populates="analysis_job", cascade="all, delete-orphan")
